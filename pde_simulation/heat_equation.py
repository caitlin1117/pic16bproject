...
def plot(u_0,pl ="animation",method ="implicit"):
    ...
    if(pl=="animation"):
        ...
        ax.set_ylim((0, 1))
        ax.set_ylabel("$u$")
        ax.set_title('$u_t = u_{xx}$ IC $u(x,0) = e^{-200(x-0.5)^2}$ homogeneous Dirichlet BCs Implicit method');
        def update(step):    
            state = u[step,:]
        ...
        plt.xlabel('x')
        plt.ylabel('t')
        plt.title('$u_t = u_{xx}$ IC $u(x,0) = e^{-200(x-0.5)^2}$ homogeneous Dirichlet BCs Implicit method')
        plt.show()
        return fig
    else:
        x = np.linspace(0, 1, N+1)
...
def plot_3d(u_0):
    ...
    ax.set_title('$u_t = u_{xx}+u_{yy}$ initial condition: random temperature between 0 and 1 homogeneous Neumann BCs');
