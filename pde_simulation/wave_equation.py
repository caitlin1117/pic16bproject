...
def plot_2d(u_0):
    ...
    ax.set_title('$u_{tt} = u_{xx}+u_{yy}$ with initial condition $u(x,y,0) = -0.8*e^{-200((x-1)^2+y^2)}\, u_t(x,y,0) = 0$ Neumann BCs');
...
def plot(u_0,pl="animation"):
    ...
    if(pl=="animation"):
        ...
        ax.set_ylim((-1,1))
        ax.set_ylabel("$u$")
        ax.set_title('$u_{tt} = u_{xx}$ with initial condition $u(x,0) = \sin(2 \pi x) \, u_t(x,0) = 0$');
        def update(step):    
            state = u[step,:]
        ...
        plt.xlabel('x')
        plt.ylabel('t')
        plt.title('$u_{tt} = u_{xx}$ with initial condition $u(x,0) = \sin(2 \pi x) \, u_t(x,0) = 0$')
        plt.show()
        return fig
    else:
        x = np.linspace(0, 1, 101)
